...
from pxcloud_api_client.exceptions import (
    ApiKeyError,
    ApiAttributeError,
    ApiTypeError,
    ApiValueError,
...
def validate_and_convert_types(input_value,
    ...
    if not valid_type:
        if (configuration
                or (input_class_simple == dict and dict not in valid_classes)):
...
def model_to_dict(model_instance, serialize=True):
    ...
    def extract_item(item):
        return (item[0],
                model_to_dict(item[1], serialize=serialize)) if hasattr(
    ...
    for prop_name in model_args:
        if prop_name not in discarded_args:
            var_name_to_model_instances[prop_name] = [self] + list(
                filter(lambda x: prop_name in x.openapi_types,
                       composed_instances))
